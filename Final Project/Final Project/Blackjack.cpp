...
void Blackjack::play(User user1)
{
    ...
    int aceAddition(int total, int aces){
        if (total==1){
            if (aces==1)
                total+=10;
        }
    ...
    int aceReduction(int total, int aces, bool& aceReduced){
        if (aceReduced==false){
            if (aces>0){
                total-=10;
                aceReduced=true;
    ...
    bool userPlay(int& total, int aces, bool& aceReduced, bool bust, bool hitAsked) {
        do {
            cout << "You Recieved: ";
            total+=hit(aces);
            bust=isBust(total);
    ...
    void dealerPlay(bool bust, bool dealerFirst, int dealerTotal, int dealerAces, bool dealerBust, bool dealerAceReduced, int& total) {
        if (bust==false){
            do {
